...
$(document).ready(function() {
    ...
    $(document).on("click", ".login__submit", function(e) {
        ...
        animating = true;
        var that = this;
        ripple($(that), e); // Trigger ripple effect
        $(that).addClass("processing"); // Add processing class for styling

        setTimeout(function() {
            $(that).addClass("success"); // Add success class
            setTimeout(function() {
                $app.show(); // Show app
                $app.css("top"); // Force reflow for CSS transition
                $app.addClass("active"); // Activate app for animations
            }, submitPhase2 - 70); // App reveals slightly before success animation ends
            setTimeout(function() {
                $login.hide(); // Hide login
                $login.addClass("inactive"); // Deactivate login for animations
                animating = false; // Reset animation flag
                $(that).removeClass("success processing"); // Remove animation classes
            }, submitPhase2);
        }, submitPhase1); // Start processing animation
    });
    ...
    class SuperformulaWireframe {
        ...
        init() {
            ...
            if (!this.container) {
                console.error("Error: '#scene-container' not found in the DOM. Please ensure your HTML has this div within the login form.");
                return;
            }
            ...
            this.renderer.domElement.addEventListener('mousedown', () => { this.isDragging = false; this.isMouseDown = true; });
            this.renderer.domElement.addEventListener('mousemove', () => { if (this.isMouseDown) this.isDragging = true; });
            this.renderer.domElement.addEventListener('mouseup', () => { this.isMouseDown = false; });
        }
        ...
        animate() {
            ...
            if (this.isMorphing) {
                ...
                if (morphProgress >= 1.0) {
                    ...
                    if(this.guiPresetController) this.guiPresetController.updateDisplay(); // Update GUI display
                } else {
